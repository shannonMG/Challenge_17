...
export const deleteFriend = async (req, res) => {
    try {
        ...
        // Prevent users from trying to remove themselves
        if (userId === friendId) {
            return res.status(400).json({ message: 'User cannot remove themselves as a friend.' });
        }
        // Convert IDs to ObjectId
        const userObjectId = new mongoose.Types.ObjectId(userId);
        const friendObjectId = new mongoose.Types.ObjectId(friendId);
        ...
        // Step 3: Remove friendId from user's friends list
        user.friends = user.friends.filter(friend => !friend.equals(friendObjectId));
        await user.save();
        // Step 4: Remove userId from friend's friends list (for bidirectional removal)
        friend.friends = friend.friends.filter(f => !f.equals(userObjectId));
        await friend.save();
